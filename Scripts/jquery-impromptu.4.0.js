...
(function($) {
    $.prompt = function(message, options) {
        ...
        $.each(message, function(statename, stateobj) {
            ...
            states += '<div id="' + $.prompt.options.prefix + '_state_' + statename + '" class="' + $.prompt.options.prefix + '_state" style="display:none;">' + arrow + '<div class="' + $.prompt.options.prefix + 'message"><table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0"><tr><td valign="middle">' + stateobj.html + '</td></tr></table></div>';

            states += '<div class="' + $.prompt.options.prefix + 'buttons"><table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0"><tr><td valign="middle">';
            $.each(stateobj.buttons, function(k, v) {
                if (typeof v == 'object')
                    buttonsList += '<button name="' + $.prompt.options.prefix + '_' + statename + '_button' + v.title.replace(/[^a-z0-9]+/gi, '') + '" id="' + $.prompt.options.prefix + '_' + statename + '_button' + v.title.replace(/[^a-z0-9]+/gi, '') + '" value="' + v.value + '" ClickId="'+v.title+'">' + v.title + '</button>';
                /*
                else
                    buttonsList += '<button name="' + $.prompt.options.prefix + '_' + statename + '_button' + k + '" id="' + $.prompt.options.prefix + '_' + statename + '_button' + k + '" value="' + v + '">' + k + '</button>';*/
                else
                    buttonsList += '<button name="' + $.prompt.options.prefix + '_' + statename + '_button' + k + '" id="' + $.prompt.options.prefix + '_' + statename + '_button' + k + '" value="' + v + '"  class="' + $.prompt.options.prefix + 'buttons' +  k + '" ClickId="'+k+'"></button>';
            });
            if (buttonsList == "") buttonsList = '&nbsp;';

            states += buttonsList;
            states += '</div></td></tr></table>';
            states += '</div>';
        });
        ...
        $.each(message, function(statename, stateobj) {
            ...
            $state.children('.' + $.prompt.options.prefix + 'buttons').find(':button').click(function() {
                ...
                var clicked = stateobj.buttons[$(this).attr("ClickId")];
                if (clicked == undefined) {
                    for (var i in stateobj.buttons)
                        //if (stateobj.buttons[i].title == $(this).text())
                        if (stateobj.buttons[i].title == $(this).attr("ClickId"))
        ...
        var keyPressEventHandler = function(e) {
            ...
            //escape key closes
            if (key == 27) {
                var $state = $.prompt.jqi.find('#' + $.prompt.options.prefix + '_state_' + $.prompt.currentStateName);
                if ($state.find('.' + $.prompt.options.prefix + 'buttons :button[value=false]').length) {
                    $state.find('.' + $.prompt.options.prefix + 'buttons :button[value=false]').click();
                    return;
                }
                if ($state.find('.' + $.prompt.options.prefix + 'buttons :button[value=true]').length) {
                    $state.find('.' + $.prompt.options.prefix + 'buttons :button[value=true]').click();
                    return;
                }
                fadeClicked();
            }
        ...
        $.prompt.jqib.bind("keypress", keyPressEventHandler);
